...
class IndexController extends AbstractActionController {
	...
	public function indexAction() {
		...
		if ($actionname == "gitpull") {
			...
			try {
				...
				/*
				 * If need server launch, guzzle to start, and set transaction as pending
				 */
	...
	protected function transcoderAction() {
		...
		if ($proceed) {
			...
			Mlog::addone ( __CLASS__ . __METHOD__ . __LINE__ . '::$action--->', $_REQUEST);
	...
	protected function returnResponse($response) {
		...
		// check headers
		if (headers_sent ()) {
			Mlog::addone ( __CLASS__ . __METHOD__, '::Success: response header 200 sucessfully sent' );
		} else {
			Mlog::addone ( __CLASS__ . __METHOD__, '::FAIL: response header 200 NOT sucessfully sent' );
		}
	}
