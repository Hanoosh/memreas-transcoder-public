	...
	public function __construct($service_locator, $aws_manager) {
		try {
			...
			try {
				...
				$this->aws_manager->sesEmailErrorToAdmin ( "backend worker error:: failed to obtain database handle " . $e->getMessage (), "error retrieveing database handle" );
				throw $e;
			}
			...
			try {
				...
				Mlog::addone ( __FILE__ . __METHOD__ . __LINE__ . 'Caught exception: ', $e->getMessage () );
				$this->aws_manager->sesEmailErrorToAdmin ( "backend worker error:: failed to obtain redis handle " . $e->getMessage (), "error retrieveing redis handle" );
				throw $e;
			}
			...
			Mlog::addone ( __FILE__ . __METHOD__ . __LINE__ . 'Caught exception: ', $e->getMessage () );
			$this->aws_manager->sesEmailErrorToAdmin ( "backend worker error:: " . $e->getMessage (), __CLASS__.'::error in __construct($service_locator, $aws_manager)' );
		}
	}
