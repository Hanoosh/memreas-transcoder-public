...
class MemreasTranscoder {
	...
	const _VERY_SMALL = '79x80/';
	const _LARGE = '448x306/';
	const _MEDIUM = '384x216/';
	const _SMALL = '98x78/';
	const _EXTRA_LARGE = '1280x720/';
	protected $user_id;
	protected $media_id;
		...
		try {
			...
			if (isset ( $message_data )) {
				...
				if ($this->is_video || $this->is_audio) {
					 ...
					 * Video Section
					 */
					// Let's use $this->MediaFileType variable to check whether uploaded file is supported.
	...
	public function createThumbnails($is_image = null) {
		try {
			...
			foreach ( $media_thumb_arr as $filename ) {
				...
				$tns_sized = array (
						"79x80" => $this->resizeImage ( $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_VERY_SMALL, $filename, basename ( $filename ), 79, 80 ),
						"448x306" => $this->resizeImage ( $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_LARGE, $filename, basename ( $filename ), 448, 306 ),
						"384x216" => $this->resizeImage ( $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_MEDIUM, $filename, basename ( $filename ), 384, 216 ),
						"98x78" => $this->resizeImage ( $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_SMALL, $filename, basename ( $filename ), 98, 78 ),
						"1280x720" => $this->resizeImage ( $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_EXTRA_LARGE, $filename, basename ( $filename ), 1280, 720 ) 
				);
	...
	public function createFolders() {
		try {
			...
			$toCreate = array (
					...
					$this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_VERY_SMALL, // data/temp_job_uuid_dir/media/thumbnails/79x80/
					$this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_LARGE, // data/temp_job_uuid_dir/media/thumbnails/448x306/
					$this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_MEDIUM, // data/temp_job_uuid_dir/media/thumbnails/384x216/
					$this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_SMALL, // data/temp_job_uuid_dir/media/thumbnails/98x78/
					$this->homeDir . self::DESTDIR . self::THUMBNAILSDIR . self::_EXTRA_LARGE, // data/temp_job_uuid_dir/media/thumbnails/1280x720/
